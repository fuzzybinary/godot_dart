...
Future<void> generateNativeStructures(
  ...
  for (Map<String, dynamic> nativeStructApi in api.nativeStructures.values) {
    String structName = nativeStructApi['name'];
    String correctedName = getCorrectedType(structName);
    if (hasDartType(structName)) {
      continue;
    }

    final destPath =
        path.join(targetDir, '${correctedName.toSnakeCase()}.dart');
    final out = File(destPath).openWrite();

    out.write(header);

    final fields = (nativeStructApi['format'] as String)
        .split(';')
...
class $correctedName extends Struct {
...
''');
    ...
    exportsString += "import '${correctedName.toSnakeCase()}.dart';\n";
  }
